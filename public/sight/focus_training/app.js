...
// ====== 5. Visual Discrimination (Concentric) ======
...
function stopBreathing() { breathState.isPlaying = false; clearTimeout(breathState.interval); document.getElementById('breathing-start').textContent = '开始呼吸'; let mins = Math.floor(breathState.totalSeconds / 60); if(mins > 0) localStorage.setItem('focus_breath_mins', (parseInt(localStorage.getItem('focus_breath_mins')||0)) + mins); updateDashboard(); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('video-start').onclick = () => {
        ...
        videoState.answer = count; let start2 = Date.now();
        const anim = () => {
            ...
            if(Date.now()-start2 < 4000) requestAnimationFrame(anim);
            else {
    ...
    document.getElementById('aud-react-start').onclick = () => { audReactState.isPlaying = true; document.getElementById('aud-react-start').disabled = true; setTimeout(() => { const c = ['red', 'blue', 'green'][Math.floor(Math.random()*3)]; audReactState.targetColor = c; Speech.speak(c==='red'?'红色':c==='blue'?'蓝色':'绿色', () => { audReactState.startTime = Date.now(); }); }, 1000 + Math.random()*2000); };
    document.querySelectorAll('.btn-circle').forEach(b => { b.onclick = () => { if(!audReactState.isPlaying || !audReactState.startTime) return; if(b.getAttribute('data-color') === audReactState.targetColor) alert(`正确！用时 ${Date.now()-audReactState.startTime}ms`); else alert("选错了！"); audReactState.isPlaying = false; audReactState.startTime = 0; document.getElementById('aud-react-start').disabled = false; }; });
    document.getElementById('aud-span-start').onclick = () => { audSpanState.isPlaying = true; document.getElementById('aud-span-start').classList.add('hidden'); audSpanState.sequence = Array.from({length: audSpanState.currentLevel}, () => Math.floor(Math.random()*10)); let i = 0; const iv = setInterval(() => { if(i < audSpanState.sequence.length) Speech.speak(audSpanState.sequence[i++].toString()); else { clearInterval(iv); setTimeout(() => { document.getElementById('aud-span-input-box').classList.remove('hidden'); const inp = document.getElementById('aud-span-input'); inp.value = ''; inp.focus(); }, 1000); } }, 1200); };
    document.getElementById('aud-span-submit').onclick = () => { if(document.getElementById('aud-span-input').value === audSpanState.sequence.join('')) { audSpanState.currentLevel++; alert("正确！"); document.getElementById('aud-span-input-box').classList.add('hidden'); document.getElementById('aud-span-start').classList.remove('hidden'); } else { alert(`失败！得分 ${audSpanState.currentLevel-1}`); localStorage.setItem('focus_aud_span_best', Math.max(audSpanState.currentLevel-1, parseInt(localStorage.getItem('focus_aud_span_best')||0))); audSpanState.currentLevel = 3; initAudSpan(); document.getElementById('aud-span-start').classList.remove('hidden'); updateDashboard(); } };
    document.getElementById('mem-rev-start').onclick = () => { const w = ['苹果','大海','蓝天','森林','书本'][Math.floor(Math.random()*5)]; Speech.speak(w, () => { const inp = document.getElementById('mem-rev-input'); inp.disabled = false; inp.value = ''; inp.focus(); inp.onkeypress = (e) => { if(e.key==='Enter') { if(inp.value === w.split('').reverse().join('')) alert("全对！"); else alert("不正确！"); initMemReverse(); } }; }); };
    document.getElementById('aud-inter-start').onclick = () => { const audioCtx = new (window.AudioContext || window.webkitAudioContext)(); const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain(); osc.connect(gain); gain.connect(audioCtx.destination); osc.type = 'sawtooth'; osc.frequency.setValueAtTime(150, audioCtx.currentTime); gain.gain.setValueAtTime(0.04, audioCtx.currentTime); const target = Array.from({length: 4}, () => Math.floor(Math.random()*10)).join(''); document.getElementById('aud-inter-start').classList.add('hidden'); osc.start(); Speech.speak(target, () => { osc.stop(); document.getElementById('aud-inter-input-box').classList.remove('hidden'); const inp = document.getElementById('aud-inter-input'); inp.value = ''; inp.focus(); inp.onkeypress = (e) => { if (e.key === 'Enter') { if (inp.value === target) alert("正确！"); else alert("失败！"); initAudInter(); } }; }); };
    document.getElementById('mem-repeat-start').onclick = () => { const target = SENTENCES[Math.floor(Math.random()*SENTENCES.length)]; document.getElementById('mem-repeat-start').classList.add('hidden'); Speech.speak(target, () => { document.getElementById('mem-repeat-input-box').classList.remove('hidden'); const inp = document.getElementById('mem-repeat-input'); inp.value = ''; inp.focus(); }); document.getElementById('mem-repeat-submit').onclick = () => { if(document.getElementById('mem-repeat-input').value.trim() === target) alert("全对！"); else alert("有偏误！"); initMemRepeat(); }; };
    document.getElementById('wec-run').onclick = () => { initAssessmentWechsler(); alert("评估报告已刷新！"); };
    document.getElementById('breathing-start').onclick = () => { if(breathState.isPlaying) stopBreathing(); else { breathState.isPlaying = true; document.getElementById('breathing-start').textContent = '停止'; const cycle = () => { if(!breathState.isPlaying) return; const c = document.getElementById('b-circle'), t = document.getElementById('b-instruction'); t.textContent = '吸气...'; c.className = 'b-circle glass inhale'; breathState.interval = setTimeout(() => { t.textContent = '呼气...'; c.className = 'b-circle glass exhale'; breathState.interval = setTimeout(() => { breathState.totalSeconds += 8; cycle(); }, 4000); }, 4000); }; cycle(); } };
});
